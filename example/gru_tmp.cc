...
void calibrateGruScales(
    ...
    dev::vector<float> x_dev(x, time_steps * batch_size * input_size);
...
void GruInference(const Tensor2f& W, const Tensor2f& R, const Tensor1f& bx,
                  const Tensor1f& br, const Tensor3f& x, Tensor3f& h) {
    ...
    device_ptr<Tensor3f> tmp_Wx_dev(time_steps * batch_size * hidden_size *
                                    3);  // 用于存放W * x的中间结果
    device_ptr<Tensor2f> tmp_Rh_dev(batch_size * hidden_size *
                                    3);  // 用于存放R * h的中间结果
...
int main() {
    ...
    Tensor1f bx(HIDDEN_DIMS * 3);  // 对应b_z/b_r/b_h的合并. bx 负责给 "输入 x_t
                                             // 到门控的线性变换" 加偏置
    Tensor1f br(HIDDEN_DIMS *
    ...
    Tensor1i32 bx_quant(HIDDEN_DIMS * 3);  // 对应b_z/b_r/b_h的合并. bx 负责给
                                                    // “输入 x_t 到门控的线性变换” 加偏置
    Tensor1i32 br_quant(HIDDEN_DIMS *
                        3);  // br: 3H(部分实现中偏置分输出\隐藏层. br
