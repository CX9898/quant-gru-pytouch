...
std::pair<float, int32_t> calculateQuantScale(T min_val, T max_val, bool symmetric,
    ...
    if (std::isnan(min_val) || std::isnan(max_val) || std::isinf(min_val) || std::isinf(max_val)) {
    ...
    if (scale <= 1e-6f) {
        printf("Warning! %s : scale = %.15f <= 1e-6f, min_val = %.15f, max_val = %.15f\n",
               name.c_str(), scale, min_val, max_val);
    }
...
//             printf("[DEBUG calculateQuantScale] min_val=%f, max_val=%f, scale=%f, zp_float=%f,
//             zp=%d (adjusted)\n",
...
template <typename T, typename QuantT>
std::vector<int8_t> calculateScalesPerChannels(const T *W_dev, int channel_size, int input_size,
                                                const std::string &name = "") {
    // 列主序排列
...
template <typename T, typename QuantT>
void calculateScale(const std::vector<T> &data_host, const bool use_symmetric, int8_t &exp2_inv,
